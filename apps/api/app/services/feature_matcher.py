...
from app.services.vector_search import ClaimSearchCandidate, ClaimSearchRecord
...
def extract_product_features(product_description: str, limit: int = 8) -> list[str]:
    clauses = [
        _clean_feature(clause)
        for clause in _CLAUSE_SPLIT_PATTERN.split(product_description)
    ]
    features: list[str] = []
    seen: set[str] = set()
...
def match_claim_element(
    ...
    if len(element_tokens) < 2:
        return FeatureMatch(
            ...
            status="uncertain",
            evidence=None,
            uncertainty="청구항 구성요소가 너무 짧아 제품 기능과 안정적으로 비교하기 어렵습니다.",
            score=0.0,
        )
    ...
    if best_score >= 0.2:
        return FeatureMatch(
            ...
            status="partial",
            evidence=evidence,
            uncertainty="일부 표현은 겹치지만 청구항의 모든 조건을 확인하기에는 제품 설명이 부족합니다.",
            score=best_score,
        )
    return FeatureMatch(
        product_feature=best_feature,
...
def generate_claim_chart_report(rows: Sequence[ClaimChartRow]) -> str:
    if not rows:
        return (
            "## 기술 검토 초안\n\n"
            "관련 청구항 후보를 찾지 못했습니다. 검색어를 구체화하거나 KIPRIS 수집 범위를 넓힌 뒤 "
            "다시 분석해야 합니다."
        )
